...
avocados <- avocados %>%
  rename(small_hass = X4046, large_hass = X4225, xlarge_hass = X4770)
...
avocados <- avocados %>% 
  mutate(other_avos = Total.Volume - small_hass - large_hass - xlarge_hass)
...
by_size <- avocados %>% 
  select(Date, other_avos, small_hass, large_hass, xlarge_hass)
